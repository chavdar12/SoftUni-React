...
type ValuesAndLoss = { values: number[]; loss: number };
...
class Color {
  ...
  linear(slope: number, intercept = 0): void {
...
class Solver {
  ...
  solveWide(): ValuesAndLoss {
    const A = 5;
    const c = 15;
    const a = [60, 180, 18000, 600, 1.2, 1.2];

    let best: ValuesAndLoss = { values: [], loss: Infinity };
